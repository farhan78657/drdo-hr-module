...
const navItems = [
  ...
  { name: 'Add Intern', path: '/hr/add-intern', icon: UserPlus },
  { name: 'Awaiting Assignment', path: '/hr/unassigned', icon: Clock },
  { name: 'Active Trainees', path: '/hr/ongoing', icon: Users },
  { name: 'Scientists', path: '/hr/scientists', icon: FlaskConical },
  { name: 'Attendance', path: '/hr/attendance', icon: Calendar },
  { name: 'Completed', path: '/hr/completed', icon: CheckCircle },
  { name: 'Certificates', path: '/hr/certificates', icon: Award },
];
...
export default function DashboardLayout() {
  ...
  useEffect(() => {
    try {
      ...
      if (userStr) {
        ...
        setUserName(parsed.name || 'HR Admin');
  ...
  return (
    <div className="min-h-screen flex bg-[#fafafa] selection:bg-zinc-100 text-zinc-900 font-sans">
      ...
      >
        ...
        <div className="border-t border-white/[0.04] p-3 space-y-2">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md text-[12px] text-zinc-500 hover:text-red-400 hover:bg-red-500/[0.04] transition-all duration-100 cursor-pointer"
          >
        ...
        <header className="sticky top-0 z-20 h-[52px] bg-white border-b border-zinc-200/50 flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex items-center gap-3 relative">
            ...
            {notificationsOpen && (
              <div className="absolute right-0 top-10 w-72 bg-white border border-zinc-200/80 rounded-md shadow-[0_4px_12px_rgba(0,0,0,0.05)] p-3.5 z-50 space-y-3">
                ...
                <div className="space-y-1">
                  {[
                    { text: 'Rahul Sharma registered as CSE candidate', time: '5m ago', path: '/hr/unassigned' },
                    { text: 'Vikram Joshi recommended for gate pass', time: '1h ago', path: '/hr/ongoing' },
                    { text: 'Dr. Verma completed evaluation for Priya', time: '2h ago', path: '/hr/completed' },
                  ].map((n, i) => (
                    <Link
                      key={i}
                      to={n.path}
            ...
            >
              ...
              <ChevronDown className="w-3 h-3 text-zinc-450" strokeWidth={1.5} />
            </button>
            ...
            {profileOpen && (
              <div className="absolute right-0 top-10 w-48 bg-white border border-zinc-200/80 rounded-md shadow-[0_4px_12px_rgba(0,0,0,0.05)] p-2.5 z-50">
                <div className="pb-2 mb-2 border-b border-zinc-100 px-1">
                  <p className="text-[12px] font-medium text-zinc-800">{userName}</p>
                  <p className="text-[10px] text-zinc-400 font-mono">{userEmail}</p>
                </div>
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-2 px-2 py-1.5 text-[11px] font-medium text-red-600 hover:bg-red-50 rounded transition-colors duration-100 cursor-pointer"
                >
                  <LogOut className="w-3.5 h-3.5" strokeWidth={1.5} />
                  <span>Sign out</span>
                </button>
